...
### 23. Ephermeral Volume

- Pod un yaşam süresinden bağımsız volum tipidir.
- Bu volum tipi cluster dışında durur ve diğertüm worker node tarafından ulaşılabilir.
- CSI ; CSI, isteğe bağlı blok ve dosya depolama depolama sistemlerini Kubernetes gibi Container Orchestration Systems (CO'ler) üzerindeki containerized iş yüklerine maruz bırakmak için bir standart olarak geliştirilmiştir.
- Bir persistent volume direk olarak bir pod ile eşleştiremiyoruz.
- Bunu bir "PVC" yaratarak yapabiliyoruz
...
- Persistent volume manual ile oluşturmak yerine claime göre otamitik oluşturmak için kullanılır.
- Örneğin hem hızlı SSD li hemde klasik HDD kullanmak istiyorsak hangi volume nin hangi depolama türünde çalışabileceğini gösterebilirim.


### 25. Stateful Set

- 3 Pod oluşturacak bir deployment objesine sahip olduğumuzu düşünelim.
- Replicaset random isimler vererek bir birinin aynısı 3 tane pod olıuşturacak. 
- Eğer pod sayısını artırabiliyorum ancak sayısını azaltmak istediğimde rast gele silme işlemi yapıyor.
- Yeni nesil NOSQL veri tabanlarında bir tane master instances vardır ve bunun üzerinden bir cluster oluşturulur.
- Bu clustera yeni instancesler eklenir.
- Yazma işlemi master dan gerçekleşirken, sorgulama işlemleri instancler üzerinden yapılır.
- Tüm veri bu instancler üzerinde dağıtık bir şekilde bulunur.
- Yapabileceğimiz işlem ile sırasıyla silme işlemi
...
metadata:
  ...
  name: cassandra
spec:
  clusterIP: None # ip i atanmıyacak
  ports:
  - port: 9042
  selector:
    app: cassandra
...
spec:
  ...
  volumeClaimTemplates: #Verilen özelliklerde pvc oluşturulmasını söylüyoruz
  - metadata:
      name: cassandra-data
    spec:
